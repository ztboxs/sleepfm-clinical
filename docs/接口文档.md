...
所有需要上传 EDF 文件的接口均支持**两种文件传入方式**（二选一）：

| 方式 | 参数 | 类型 | 说明 |
|------|------|------|------|
| **直接上传** | `file` | `file` | 通过 multipart/form-data 直接上传 EDF 文件 |
| **URL 下载** | `file_url` | `string` | 提供 EDF 文件的下载地址，服务端自动下载 |

> 适用场景：对接的平台（如 Coze、Dify 等工作流平台）不支持 `file` 类型参数时，可将 EDF 文件存放在可访问的地址（对象存储、文件服务器等），通过 `file_url` 传入。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `file` | `file` | 二选一 | 直接上传 PSG 记录文件，仅接受 `.edf` 格式 |
| `file_url` | `string` | 二选一 | EDF 文件的下载 URL，服务端自动下载 |

> `file` 和 `file_url` 必须提供其中一个。
...
| `embeddings.<MOD>.shape` | `array[integer]` | 嵌入张量维度 `[时间步数, 嵌入维度]`，如 `[1740, 128]` |
...
上传 EDF 文件，执行完整推理流水线：预处理 → 嵌入生成 → 睡眠分期和/或疾病风险预测。这是最常用的接口，一次调用获取所有分析结果。
...
| `min_hazard_score` | `float` | 否 | 不过滤 | 疾病风险评分最低阈值，仅返回 `hazard_score >= min_hazard_score` 的疾病。不传则返回全部 1065 种。建议值：`7` |
...
| `predicted_stage` | `string` | 预测的睡眠阶段：`Wake`/`N1`/`N2`/`N3`/`REM` |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `top_risks` | `array[DiseaseRiskItem]` | 按风险评分降序排列的疾病风险列表。默认返回全部 1065 种；若传入 `min_hazard_score` 则仅返回达到阈值的疾病 |
...
**仅睡眠分期**（`tasks=sleep_staging`）：响应中 `disease_prediction` 为 `null`。

**仅疾病预测**（`tasks=disease_prediction`）：响应中 `sleep_staging` 为 `null`，且必须提供 `age` 和 `gender`。
...
```
{
  "top_risks":         [DiseaseRiskItem],  // 疾病风险列表 (按 hazard_score 降序)
...
```bash
curl -X POST http://<服务器IP>:6006/api/v1/predict \
  -F "file_url=https://your-storage.com/path/to/recording.edf" \
  -F "age=0.45" \
  -F "gender=1" \
  -F "tasks=sleep_staging,disease_prediction"
```
...
# 方式二：通过 URL 传入文件（适用于不支持文件上传的平台）
resp = requests.post(
    f"{BASE_URL}/api/v1/predict",
    data={
        "file_url": "https://your-storage.com/path/to/recording.edf",
        "age": 0.45,
        "gender": 1,
        "tasks": "sleep_staging,disease_prediction",
    },
...
formData2.append('file_url', 'https://your-storage.com/path/to/recording.edf');
formData2.append('age', '0.45');
formData2.append('gender', '1');
formData2.append('tasks', 'sleep_staging,disease_prediction');

const resp2 = await fetch('http://<服务器IP>:6006/api/v1/predict', {
  method: 'POST',
  body: formData2,
});
